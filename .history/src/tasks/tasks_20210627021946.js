...
import { Button,Input,Grid,Paper,borders } from "@material-ui/core";
        ...
        <Grid container spacing={1} className={classes.root}>
            <Grid item xs={12}>
                <Paper className={classes.paper}>
                    ...
                    <Button type="submit" color="primary" variant="contained" onClick={addTask} className={classes.buttons}>
                       <AddIcon/>
                    </Button>
                        <ul className={classes.lists}>
                            { newtask.map((val, key)=>{
                                return <li key={key} borders={1}>{val}</li> 
                                })
                            }
                            </ul>
                    <div>
